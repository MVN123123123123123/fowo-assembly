...
section .data
    ...
    tool_grep db "out=$( [ -d %2$s ] && echo %2$s/config || echo %2$s ); for t in bc diff bison flex perl python3 rsync m4 patch gettext autoconf automake autoreconf aclocal libtool libtoolize; do if grep -qsE ", 34, "(^|[[:space:]]|\b)$t(\b|[[:space:]]|$)", 34, " %1$s/Makefile %1$s/Kbuild %1$s/bootstrap %1$s/autogen.sh %1$s/configure.ac %1$s/configure.in 2>/dev/null; then if ! command -v $t >/dev/null 2>&1; then echo ", 34, "# Detected: $t", 34, " | sudo tee -a $out >/dev/null 2>&1; fi; fi; done", 0
    install_deps_cmd db "out=$( [ -d %2$s ] && echo %2$s/config || echo %2$s ); [ -f $out ] && awk 'BEGIN { map[", 34, "diff", 34, "]=", 34, "diffutils", 34, "; map[", 34, "autoreconf", 34, "]=", 34, "autoconf", 34, "; map[", 34, "aclocal", 34, "]=", 34, "automake", 34, "; map[", 34, "autoheader", 34, "]=", 34, "autoconf", 34, "; map[", 34, "libtoolize", 34, "]=", 34, "libtool", 34, " } /^ALIAS / { pkg=$2; for(i=4;i<=NF;i++){ val=$i; sub(/,$/,", 34, 34, ",val); map[val]=pkg } } /^SYS / { sys_map[$3]=$2 } /^# Detected:/ { dep=$3; detected[dep]=1 } END { for(dep in detected) { if(map[dep]!=", 34, 34, ") dep=map[dep]; to_install[dep]=1 } for(pkg in to_install) { if(system(", 34, "test -f /var/lib/fowo/packages/", 34, " pkg ", 34, ".db || test -f /tmp/fowo_packages/", 34, " pkg ", 34, ".db", 34, ")==0) continue; if(sys_map[pkg]!=", 34, 34, ") { pm=sys_map[pkg]; if(pm==", 34, "dnf", 34, ") cmd=", 34, "sudo dnf install -y ", 34, " pkg; else if(pm==", 34, "apt", 34, ") cmd=", 34, "sudo apt-get install -y ", 34, " pkg; else if(pm==", 34, "tce-load", 34, ") cmd=", 34, "tce-load -wi ", 34, " pkg; else cmd=pm ", 34, " install ", 34, " pkg; system(cmd); } else { system(", 34, "if command -v tce-load >/dev/null 2>&1; then tce-load -wi ", 34, " pkg ", 34, "; else %1$s install ", 34, " pkg ", 34, "; fi", 34, "); } } }' $out || true", 0
